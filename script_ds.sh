...
installChaincode () {
  ...
  VERSION=${3:-1.2}
        set -x
  peer chaincode install -n myccds -v ${VERSION} -l ${LANGUAGE} -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02 >&log.txt
  res=$?
        set +x
  cat log.txt
...
instantiateChaincode () {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
    peer chaincode instantiate -o orderer.ptunstad.no:7050 -C $CHANNEL_NAME -n myccds -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init","a","100","b","200"]}' -P "OR('Org1MSP.member','Org1MSP.member')" >&log.txt ## '{"Args":["c","asdf"]}'
    res=$?
                set +x
  else
                set -x
